...
uses Lib, TypInfo, uPSI_JavascriptObject, uPSI_DOM,
PluginBase;
...
begin
  ...
  win := THTMLWindowElement.Create;
  win.Element := (Browser.Document as ihtmldocument2).parentWindow;
  s := win.Call('callFunc', ['merhaba', 'deneme']);
  Supports(IDispatch(s), IDispatchEx, valueex);
  valueex.GetDispID('merhaba', fdexNameCaseSensitive, DispatchIdOfProperty);
  valueex.InvokeEx(DispatchIdOfProperty, LOCALE_USER_DEFAULT, DISPATCH_PROPERTYGET, @Params, vPropertyValue, Temp, nil);
  ShowMessage(vPropertyValue);
end;
